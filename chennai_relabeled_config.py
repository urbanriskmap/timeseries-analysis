...
def __get_flood_pkeys(start_date, end_date, engine):
    ...
    import os
    path = "/home/abrahamq/transfer_learning_trial/ch_flooding/train/no_flood"
    files = os.listdir(path)
    path = "/home/abrahamq/transfer_learning_trial/ch_flooding/val/no_flood"
...
def __get_no_flood_pkeys(start_flood_date, end_flood_date, engine):
    ...
    pkeys = pd.read_sql_query(
        ...
        con=engine, index_col="pkey")
    path = "/home/abrahamq/transfer_learning_trial/ch_flooding/train/heavy_flood"
    import os
    files = os.listdir(path)
    path = "/home/abrahamq/transfer_learning_trial/ch_flooding/val/heavy_flood"
    files.extend(os.listdir(path))
    keep = set()
    for each in files:
        num = each.split(".")[0]
