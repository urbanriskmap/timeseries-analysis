...
DEFAULT_LABEL = './image_recognition/min_confidence.p'
...
def perf_given_categories(allowed_cat={'Flood': 0,
                          'Water': 1, 'Puddle': 2, 'Person': 3},
                          plot=False,
                          top_lab=30,
    ...
    num_allowed = len(list(allowed_cat))
    # don't include zero data - first row is pkey of data
    matrix_w_pkey = aws.make_matrix_rep(vects, num_allowed)
    ...
    top_n = num_allowed if top_lab > num_allowed else top_lab
    ...
    max_labels = [(reverse_allowed[lab], th[lab, 0]) for lab in max_indicies]
    if (plot):
